...
def test_user2(client):
    user_data = {"email": "user1@gmail.com", "password": "user1password"}
    res = client.post("/users/", json=user_data)

    assert res.status_code == 201

    new_user = res.json()
    new_user["password"] = user_data["password"] # adds password field to res
    return new_user


@pytest.fixture
def test_user(client):
    user_data = {"email": "user2@gmail.com", "password": "user2password"}
    res = client.post("/users/", json=user_data)
    assert res.status_code == 201
    new_user = res.json()
    new_user["password"] = user_data["password"]  # adds password field to res
